...
@interface OLShortcutView () // Private accessors
...
- (BOOL)isNumpad:(NSUInteger)keyCode {
        switch (keyCode) {
        // Keypad
...
- (void)setRecording:(BOOL)flag
{
    ...
    static OLShortcutView *currentRecorder = nil;
...
- (void)drawRect:(CGRect)dirtyRect
{
    if (self.shortcutValue) {
        ...
        NSString *title = (self.recording
                           ...
                           : _shortcutValue ? _shortcutValue.olDescription : @"");
...
- (void)activateEventMonitoring:(BOOL)shouldActivate
{
    ...
    if (shouldActivate) {
        __unsafe_unretained OLShortcutView *weakSelf = self;
        ...
        eventMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:eventMask handler:^(NSEvent *event) {
            ...
            // If the shortcut is Cmd-W or Cmd-Q, cancel recording and pass the event through
                ...
                if (shortcut.keyCodeString.length > 0) {
                    //NSLog(@"keycode: %lu %lu %@ %@", (unsigned long)shortcut.keyCode, (unsigned long)shortcut.modifierFlags, shortcut.modifierFlagsString, shortcut);
                    if ([self isNumpad:shortcut.keyCode] || [_shortcutValidator isShortcutValid:shortcut]) {
